...
static unsigned char HD_register;
static unsigned char memory_register;
...
// our hd array
static unsigned char hardDrive[4096];
...
Phase write_back()
{
  ...
  switch( opcode() )
  {
    ...
    case MOVE_OPCODE:
      ...
      { // memory
        ...
        {
            if (tag == 0)
                status_register = state.MDR;
            else if (tag == 1)
                HD_register = state.MDR;
            else if (tag == 2)
                memory_register = state.MDR;
            else
                rc = cache_write();
        }
        else
  ...
  for ( int i=0 ; i<CACHE_BLOCKS ; i++ ) // This loop makes the program write-through
      write_block( i );
...
Phase interrupt()
{
    ...
    unsigned char tagForData = addr2tag( memory_register );
    unsigned char offsetForData = addr2offset( memory_register );
    unsigned char amountLeftToTransfer = status_register >> 1;
    unsigned char readOrWrite = status_register & 0x0001;
    bool blockInCache = false; // This is for write-invalidation. If this becomes true, we will update the cache block with the new data block

    if (readOrWrite) // write to the HD
    {

        if (amountLeftToTransfer == 1)
        {
            hardDrive[HD_register] = data[tagForData][offsetForData][0];
            hardDrive[HD_register] = hardDrive[HD_register] << 8;
            hardDrive[HD_register] = hardDrive[HD_register] | data[tagForData][offsetForData][1];
            amountLeftToTransfer = amountLeftToTransfer - 1;
        }
        else if (amountLeftToTransfer == 2)
        {
            for (int i = 0; i < 2; i++)
            {
                if (offsetForData < BLOCK_SIZE)
                {
                    hardDrive[HD_register] = data[tagForData][offsetForData][0];
                    hardDrive[HD_register] = hardDrive[HD_register] << 8;
                    hardDrive[HD_register] = hardDrive[HD_register] | data[tagForData][offsetForData][1];
        ...
        else if (amountLeftToTransfer == 3)
        {
            for (int i = 0; i < 3; i++)
            {
                if (offsetForData < BLOCK_SIZE)
        ...
        else if (amountLeftToTransfer >= 4)
        {
            for (int i = 0; i < 4; i++)
            {
                if (offsetForData < BLOCK_SIZE)
    ...
    else // read from the HD
    {
        if (amountLeftToTransfer == 1)
        {
            data[tagForData][offsetForData][0] = hardDrive[HD_register] >> 8;
            data[tagForData][offsetForData][1] = hardDrive[HD_register] & 0x00ff;
            amountLeftToTransfer = amountLeftToTransfer - 1;
        }
        else if (amountLeftToTransfer == 2)
        {
    ...
    memory_register = tagForData | offsetForData;
    amountLeftToTransfer = amountLeftToTransfer << 1;
    status_register = amountLeftToTransfer | readOrWrite;
    return rc;
}
...
void initialize_system()
{
  ...
  if(pFile != NULL)
  {
    fread(hardDrive, sizeof(char), 4096, pFile);
    fclose(pFile);
...
int main (int argc, const char * argv[])
{
  ...
  if ( load_files( argv[1], argv[2] ) )
  {
    ...
    pFile = fopen("hd", "wb");
    fwrite(hardDrive, sizeof(char), 4096, pFile);
    fclose(pFile);
